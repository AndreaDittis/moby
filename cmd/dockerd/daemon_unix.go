...
func (cli *DaemonCli) initContainerD() (context.Context, context.CancelFunc, error) {
	ctx, cancel := context.WithCancel(context.Background())
	if cli.Config.ContainerdAddr == "" {
		systemContainerdAddr, ok, err := systemContainerdRunning(cli.Config.IsRootless())
		if err != nil {
			cancel()
			return nil, nil, errors.Wrap(err, "could not determine whether the system containerd is running")
		}
		if !ok {
			opts, err := cli.getContainerdDaemonOpts()
			if err != nil {
				cancel()
				return nil, nil, errors.Wrap(err, "failed to generate containerd options")
			}

			r, err := supervisor.Start(ctx, filepath.Join(cli.Config.Root, "containerd"), filepath.Join(cli.Config.ExecRoot, "containerd"), opts...)
			if err != nil {
				cancel()
				return nil, nil, errors.Wrap(err, "failed to start containerd")
			}
